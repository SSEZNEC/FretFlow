...
class FingeringEngine:
    ...
    def positions_at(
        ...
        """Fret positions near the playhead — O(window), not O(all notes)."""
        if not notes:
            return []

        # Binary search on sorted notes (by start_seconds) — no full scan
        import bisect
        lo = bisect.bisect_left(notes, time_seconds - 1.5, key=lambda n: n.start_seconds)
        hi = bisect.bisect_right(notes, time_seconds + lookahead_seconds, key=lambda n: n.start_seconds)
        ...
        for note in notes[lo:hi]:
            ...
            elif -max(note.duration_seconds, 0.15) <= delta < -window:
